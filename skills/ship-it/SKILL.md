...
**dev-flow is the front-of-house orchestrator; this skill is its ship authorization.** The `dev-flow` master skill is the reasoning layer that routes a prompt (via `goto-project`), classifies it bug/feature/other, and drives the full investigate→reproduce→fix→prove lifecycle. When that flow reaches the ship boundary, it *calls this skill* for the ship semantics below — the worktree discipline, the confidence gate, the PR-then-merge decision, the teardown. dev-flow owns the "what to build and how to prove it"; ship-it owns "is it safe to merge, and how do we merge it without stranding work."
