...
description: Publish a rendered explainer video to YouTube programmatically via the YouTube Data API v3. Uploads the MP4, SRT captions, a Gemini-generated thumbnail, sets privacy + AI-content disclosure, adds to a playlist, and pins a comment linking back to the source article. Invoke when the user wants to "upload to YouTube", "publish video to youtube", "distribute video", or after an article-to-video render completes.
---

# Video Distribute

YouTube-first distribution for rendered explainer videos. Works from the same scene-script JSON the article-to-video skill consumes.
...
1. https://console.cloud.google.com → new project `glp3-wiki-distribution`
...
5. Save to `~/.config/glp3-wiki/youtube-client-secret.json`
...
```
Opens a browser → pick account → grant scopes. Refresh token is saved to `~/.config/glp3-wiki/youtube-token.json` (tokens never go into the repo).

## Environment

- `YOUTUBE_CLIENT_SECRET_PATH` — default `~/.config/glp3-wiki/youtube-client-secret.json`
- `YOUTUBE_TOKEN_PATH` — default `~/.config/glp3-wiki/youtube-token.json`
- `GEMINI_API_KEY` — required for thumbnail generation

## One-time font install (thumbnail brand mark)

The thumbnail brand mark renders "GLP-3 Wiki" in Inter Bold (matches the site nav). Install once:
...
- **Brand matches site nav**: tailwind blue-500 dot + Inter Bold "GLP-3 Wiki" in near-black (not all-caps)
...
- **Scene script JSON** — must have `slug`, `video_title`, `video_description_draft`. Chapters should already be inlined into the description draft (article-to-video does this).
- **Rendered MP4** — any resolution YouTube accepts. Article-to-video emits 1920×1080.
...
For glp3.wiki-specific editorial (YMYL disclaimer, default tags, default playlist), use the project-level `publish-video` skill, which invokes this one.
