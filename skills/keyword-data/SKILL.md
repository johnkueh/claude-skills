---
name: keyword-data
description: DataForSEO keyword research for SEO and content planning. Get search volume, search intent, keyword difficulty, CPC, and keyword suggestions. Use for keyword research, content planning, and SEO strategy. Supports --dry-run to preview costs before executing.
---
...
```
~/Projects/claude-skills/skills/keyword-data
```
...
cd ~/Projects/claude-skills/skills/keyword-data
...
#### Check balance (FREE)
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python cli.py balance
```

#### Show cost estimates
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python cli.py costs
```

#### Get search volume + intent + CPC (~$0.077 for up to 1000 keywords)
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python cli.py volume "keyword1" "keyword2" "keyword3"
cd ~/Projects/claude-skills/skills/keyword-data && uv run python cli.py volume -d "keyword1"  # with difficulty
cd ~/Projects/claude-skills/skills/keyword-data && uv run python cli.py volume --no-intent "keyword1"  # skip intent
```
...
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python cli.py suggestions "seed keyword" --limit 100
```

#### Get related keywords (~$0.02-0.04, includes clickstream-refined volumes)
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python cli.py related "keyword"
```
...
#### List result files
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python analyze.py list-files
```

#### Summarize volume results
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python analyze.py summary results/volume_*.csv --min-volume 10
```

#### Combine suggestion files
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python analyze.py combine
```

#### Find new keywords not in existing data
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python analyze.py find-new new_keywords.txt -e results/volume_existing.csv
```

#### Generate markdown report
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python analyze.py report results/volume_*.csv -o report.md
```
...
```
~/Projects/claude-skills/skills/keyword-data/results/
```
...
```bash
cd ~/Projects/claude-skills/skills/keyword-data && uv run python cli.py volume --dry-run "keyword1" "keyword2"
```
