...
| recipes, recipes.im | `~/Projects/recipes.im` | Expo app, default branch is **master**. OTA releases need per-release authorization; EAS builds use the `production` profile only. |
...
`marky.im`, `drafty.im-og`, `drafty.im-improvements`, `drafty-art-proto`, `sveds.com`, `recipes.im-server-v2`, `recipes.im-wide-events`, `ledgersignal3/4/5`, `archived/`, `Projects/`. These are old copies or experiments that shadow the real repos above. "marky" always means `drafty.im` (the Marky rename was reverted 2026-06-07).

## Maintenance

When a new project appears under ~/Projects or a row here turns out wrong, update this table in `~/Projects/claude-skills/skills/goto-project/SKILL.md` (the marketplace source), then republish the plugin (version bump + `claude plugin update goto-project@johnkueh-skills` + reload).
