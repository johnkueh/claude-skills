---
name: serp-data
description: DataForSEO SERP analysis for geo-targeted search result analysis. See who ranks for keywords in specific locations (Australia, US, UK, etc.), identify content gaps, analyze SERP features (featured snippets, PAA), and find competitor domains. Supports --dry-run to preview costs.
---
...
```
~/Projects/claude-skills/skills/serp-data
```
...
```bash
cd ~/Projects/claude-skills/skills/serp-data
...
uv run python cli.py gaps ledgersignal.com "liquidator" "rocap form" "form 507"
```
...
# Find where LedgerSignal doesn't rank
uv run python cli.py gaps ledgersignal.com "liquidator" "doca" "form 507" "rocap"
...
```bash
cd ~/Projects/claude-skills/skills/serp-data && uv run python cli.py balance
```

### 6. `costs` - Show cost estimates (FREE)

```bash
cd ~/Projects/claude-skills/skills/serp-data && uv run python cli.py costs
```

### 7. `locations` - List location shortcuts (FREE)

```bash
cd ~/Projects/claude-skills/skills/serp-data && uv run python cli.py locations
```
...
```
~/Projects/claude-skills/skills/serp-data/results/
```
...
- `gaps_ledgersignal.com_au_2026-02-05_145230.csv`
...
uv run python cli.py gaps ledgersignal.com \
...
uv run python cli.py gaps ledgersignal.com "liquidator" "doca"
```
