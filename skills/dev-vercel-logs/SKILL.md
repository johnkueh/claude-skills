...
`dev-vercel-logs` is still right for these cases even when bq-analytics is installed:
...
For projects without bq-analytics, `dev-vercel-logs` is the canonical tool — keep reading.
