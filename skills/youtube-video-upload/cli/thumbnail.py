...
Two-step pipeline:
  ...
  2. Pillow overlays:
       ...
       - Brand mark bottom-right: blue circle + brand text in Inter Bold
...
Design decisions (derived from 2026 thumbnail research + iteration):
  ...
  - Brand mark matches the site nav: tailwind blue-500 circle + Inter Bold
    brand text (from --brand flag) in near-black.
...
Defaults (pull from script.thumbnail or scene[0].overlay):
  ...
  brand    → (from --brand flag or scene JSON)
...
def derive_text(script: dict) -> tuple[str, str, str]:
    ...
    brand = thumb.get("brand") or os.environ.get("YOUTUBE_BRAND")
...
@click.option("--brand", default=None, help="Brand mark text (required, or set YOUTUBE_BRAND env)")
